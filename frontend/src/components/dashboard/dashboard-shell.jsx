...
export function DashboardShell({ initialTab }) {
  ...
  const { user, role: contextRole, switchRole } = useAuth()
  const [localRole, setLocalRole] = useState(null)
  const [overrideTab, setOverrideTab] = useState(null)
  const [sidebarOpen, setSidebarOpen] = useState(true)
  const [mobileSidebarOpen, setMobileSidebarOpen] = useState(false)

  const activeRole = localRole || contextRole || "student"
    ...
    <div className="flex h-dvh overflow-hidden bg-background">
      ...
      <div className="flex min-w-0 flex-1 flex-col transition-all duration-300">
        ...
        <main className="flex-1 overflow-y-auto p-4 md:p-6">
            ...
            <div className="max-w-3xl space-y-6">
              <div className="rounded-2xl border border-border bg-card p-6 shadow-sm">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <h2 className="text-xl font-bold text-foreground">{user?.name || "Aarav Sharma"}</h2>
                    <p className="text-sm text-muted-foreground">{user?.detail || "B.Tech Computer Science · Semester 5"}</p>
                    <span className="mt-1 inline-flex items-center gap-1.5 rounded-full bg-emerald-500/10 px-2.5 py-0.5 text-xs font-medium text-emerald-600 dark:text-emerald-400">
                      <CheckCircle2 className="size-3.5" /> Active Member ({activeRole})
                    </span>
                  </div>
                </div>
                ...
                <div className="mt-6 grid grid-cols-1 gap-4 sm:grid-cols-2">
                  <div className="rounded-xl border border-border/80 p-3.5 bg-muted/20">
                    <p className="text-xs text-muted-foreground">User / Student ID</p>
                    <p className="text-sm font-semibold text-foreground">{user?.studentId || "STU-2026-0842"}</p>
                  </div>
                  <div className="rounded-xl border border-border/80 p-3.5 bg-muted/20">
                    <p className="text-xs text-muted-foreground">Email</p>
                    <p className="text-sm font-semibold text-foreground">{user?.email || "aarav.sharma@student.univ.edu"}</p>
                  </div>
                </div>
              </div>
            </div>
          ...
          {activeTab === "Dashboard" && (
            <>
              {activeRole === "student" && <StudentDashboard onNavigate={handleNavigate} />}
