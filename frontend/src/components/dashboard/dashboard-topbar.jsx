...
export function DashboardTopbar({
  ...
  const activeRole = role || currentRole || "student"
  const meta = user
    ? {
    ...
    : roleMeta[activeRole] || roleMeta.student
          ...
          <DropdownMenuContent align="end" className="w-44">
            <DropdownMenuGroup>
              ...
              {Object.keys(roleMeta).map((r) => (
                <DropdownMenuItem key={r} onClick={() => handleRoleSelect(r)} className="cursor-pointer">
                  {roleMeta[r].label}
                </DropdownMenuItem>
              ))}
            </DropdownMenuGroup>
